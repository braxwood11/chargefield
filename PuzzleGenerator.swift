...
class PuzzleGenerator {
    ...
    static func generateRandomPuzzle(gridSize: Int = 5, difficulty: String = "medium", positiveMagnets: Int = 3, negativeMagnets: Int = 3) -> PuzzleDefinition {
        ...
        // Step 3: Select target cells (cells with clues)
        let targetCellCount = Int.random(in: targetCellRange)
        var targetValues = Array(repeating: Array(repeating: -99, count: gridSize), count: gridSize)
        
        // Strategy for selecting cells based on difficulty
        switch difficulty.lowercased() {
        case "easy":
            // For easy, select cells more evenly across the grid
            selectEvenlyDistributedTargets(targetValues: &targetValues, fieldValues: fieldValues, count: targetCellCount, gridSize: gridSize)
        case "hard":
            // For hard, select cells with some clustering
            selectClusteredTargets(targetValues: &targetValues, fieldValues: fieldValues, count: targetCellCount, gridSize: gridSize)
        default:
            // For medium, use a balanced approach
            selectBalancedTargets(targetValues: &targetValues, fieldValues: fieldValues, count: targetCellCount, gridSize: gridSize)
        }
    ...
    private static func selectEvenlyDistributedTargets(targetValues: inout [[Int]], fieldValues: [[Int]], count: Int, gridSize: Int) {
        ...
        for position in allPositions {
            ...
            if regionMap[regionRow][regionCol] < maxPerRegion {
                targetValues[position.row][position.col] = fieldValues[position.row][position.col]
        ...
        if selectedCount < count {
            for position in allPositions {
                if targetValues[position.row][position.col] == -99 { // Not yet assigned
                    targetValues[position.row][position.col] = fieldValues[position.row][position.col]
                    selectedCount += 1
    ...
    private static func selectClusteredTargets(targetValues: inout [[Int]], fieldValues: [[Int]], count: Int, gridSize: Int) {
        ...
        for entry in positionsWithDistances {
            ...
            if Double.random(in: 0...1) > 0.2 || selectedCount < count / 2 { // Ensure we select at least half the required count
                ...
                if targetValues[row][col] == -99 { // Not yet assigned
                    targetValues[row][col] = fieldValues[row][col]
                    selectedCount += 1
    ...
    private static func selectBalancedTargets(targetValues: inout [[Int]], fieldValues: [[Int]], count: Int, gridSize: Int) {
        ...
        for position in sortedByAbsValue.prefix(extremeValueCount) {
            targetValues[position.row][position.col] = position.value
            selectedCount += 1
        }
        ...
        var remainingPositions = allPositions.filter { targetValues[$0.row][$0.col] == -99 }
